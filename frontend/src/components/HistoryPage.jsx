...
export default function HistoryPage({ history }) {
  ...
  const filtered = history.filter(h => {
    const matchFilter = filter === 'all' || h.decision?.decision?.toLowerCase().replace(' ','') === filter
  ...
  return (
    <div className="fade-in">
      <div className="page-header">
        ...
        <p className="page-subtitle">Complete log of all AI decisions with reasoning and confidence scores</p>
      </div>
          ...
          <div style={{ display:'flex', gap:'10px', marginBottom:'16px', flexWrap:'wrap', alignItems:'center' }}>
            ...
            {['all','approved','rejected','needsreview'].map(f => (
              <button key={f}
                ...
                {f === 'all' ? 'All' : f === 'needsreview' ? 'Review' : f.charAt(0).toUpperCase()+f.slice(1)}
              </button>
            ))}
            {history.length > 0 && (
              <button className="btn btn-secondary" onClick={exportCSV} style={{ padding:'8px 14px', fontSize:'12px' }}>
                ↓ Export CSV
              </button>
            )}
          </div>
          ...
          <div className="card">
            {filtered.length === 0 ? (
              <div className="empty-state">
                ...
                <div className="empty-title">{history.length === 0 ? 'No history yet' : 'No matches found'}</div>
                <div className="empty-sub">{history.length === 0 ? 'Analyzed documents will appear here' : 'Try a different filter'}</div>
              </div>
            ) : (
              <table className="history-table">
                <thead>
                  <tr>
                    <th>Document</th><th>Category</th><th>Decision</th>
                ...
                <tbody>
                  {filtered.map(entry => (
                    <tr key={entry.id}
                      ...
                      style={{ cursor:'pointer', background: selected?.id === entry.id ? 'var(--surface2)' : '' }}>
                      ...
                      <td style={{ fontFamily:'DM Mono,monospace', color:'var(--accent)' }}>{entry.decision?.confidence}%</td>
          ...
          <div className="fade-in">
            <div className="card" style={{ marginBottom:'16px' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'16px' }}>
                ...
                <button className="btn btn-secondary" onClick={() => setSelected(null)} style={{ padding:'6px 12px', fontSize:'12px' }}>✕ Close</button>
              </div>
              <span className={`decision-badge ${
                selected.decision?.decision === 'Approved' ? 'badge-approved' :
                selected.decision?.decision === 'Rejected' ? 'badge-rejected' : 'badge-review'
              }`}>{selected.decision?.decision}</span>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px', margin:'16px 0' }}>
                <div style={{ padding:'12px', background:'var(--surface2)', borderRadius:'8px', border:'1px solid var(--border)' }}>
                  ...
                  <div style={{ fontSize:'28px', fontFamily:'DM Serif Display,serif', color:'var(--accent)' }}>{selected.decision?.confidence}%</div>
                </div>
                <div style={{ padding:'12px', background:'var(--surface2)', borderRadius:'8px', border:'1px solid var(--border)' }}>
                  ...
                  <div style={{ fontSize:'28px', fontFamily:'DM Serif Display,serif', color:
              ...
              <div style={{ padding:'10px', background:'var(--surface2)', borderRadius:'6px', marginBottom:'12px', border:'1px solid var(--border)' }}>
            ...
            <div className="card">
              ...
              <div className="reasoning-box">{selected.decision?.reasoning}</div>
            </div>
          </div>
        )}
      </div>
